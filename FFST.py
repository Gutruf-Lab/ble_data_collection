...
output_file_name = ""
friendly_name = "Dual Humidity 1"

column_names = ["Time:", "Temperature1 (Celsius):", "Relative Humidity1 (Pct):", "Temperature2 (Celsius):",
                "Relative Humidity2 (Pct):"]
...
def sht3x_notification_handlertop(sender, data):
    ...
    temperature1 = x[0] / 1000
    rel_humidity1 = x[1] / 1000
    ...
    packaged_data = {
        "Time:": [time.time()],
        "Temperature1 (Celsius):": temperature1,
        "Relative Humidity1 (Pct):": rel_humidity1,
    ...
    #data_df.to_csv(output_file_name, index=False, header=False, mode='a')
    new_df.to_csv(output_file_name, index=False, header=False, mode='a')


def sht3x_notification_handlerbottom(sender, data):
    global output_file_name
    # global data_df
    # global xs
    # global ys
    ...
    x = struct.unpack('<II'.format(len(data)), data)
    # data_str = x.decode('utf-8')
    temperature2 = x[0] / 1000
    rel_humidity2 = x[1] / 1000

    packaged_data = {
        "Time:": [time.time()],
        "Temperature1 (Celsius):": "",
        "Relative Humidity1 (Pct):": "",
        "Temperature2 (Celsius):": temperature2,
        "Relative Humidity2 (Pct):": rel_humidity2,
    }
    print(packaged_data)

    new_df = pd.DataFrame(packaged_data, columns=column_names)
    #data_df = pd.concat([data_df, new_df], ignore_index=True)
    #data_df.to_csv(output_file_name, index=False, header=False, mode='a')
    new_df.to_csv(output_file_name, index=False, header=False, mode='a')
    ...
    while True:
        try:
            ...
            async with BleakClient(address, disconnected_callback=disconnect_callback) as client:
                ...
                await client.start_notify('2d86686a-53dc-25b3-0c4a-f0e10c8def11', sht3x_notification_handlertop)
                await client.start_notify('2d86686a-53dc-25b3-0c4a-f0e10c8def12', sht3x_notification_handlerbottom)
...
def create_csv_if_not_exist(filename_address):
    ...
    if not path.exists(output_file_name):
        ...
        new_file_headers = pd.DataFrame(
            columns=['Time:', "Temperature1 (Celsius):", "Relative Humidity1 (Pct):", "Temperature2 (Celsius):",
                     "Relative Humidity2 (Pct):"])
